...
本模块负责：
1. 计算综合成本 cost = cny_hkd * usd_hkd；
2. 将每日结果写入 history_rates.csv；
3. 若当天已有记录，则执行更新而不是无限追加，避免同一天重复运行导致样本失真。
"""
...
def save_rate_record(
    ...
    """将最新汇率结果写入 CSV，并返回写入后的完整 DataFrame。"""
    ...
    if history_df.empty:
        ...
        if same_day_mask.any():
            history_df.loc[same_day_mask, CSV_COLUMNS[1:]] = [
                new_record["cny_hkd"],
                new_record["usd_hkd"],
                new_record["cost"],
            ]
